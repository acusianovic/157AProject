function [newrocket] = getWeightLength(newrocket)
    ...
    %% Nosecone Weight
    L_nosecone = newrocket.geo.nc.L*12; % inches
    rho_nosecone = 100;        % Fiberglass (lb/ft^3), change later
    W_nosecone = rho_nosecone*newrocket.geo.nc.V; % ft3
    ...
    V_body = (pi/4) * ((D+newrocket.geo.nc.tn)^2 - D^2);
...
%     Weight(5) = W_dry;
    ...
    newrocket.data.length.L = L_nosecone + L_body; % total length, inches
    ...
    newrocket.geo.length.body = L_body;    % Body lengthm inches
