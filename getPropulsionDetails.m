...
V_press = (V_ox.*rocket.prop.P_ox + V_fuel.*rocket.prop.P_fuel)/rocket.prop.P_press; % total required pressurant volume at press tank pressure, in3
V_press_STP = rocket.prop.P_press/14.7; % vol at STP, ft3 or SCF 
...
mtot = m_ox + m_oxtank + m_fuel + m_fueltank + m_press + m_presstank + m_chamber; % lbm
[~,Ind] = min(mtot);
...
rocket.prop.Vc = m_chamber(Ind);
rocket.prop.Ac = A_chamber(Ind);
rocket.prop.tc = t_chamber(Ind);
rocket.prop.mc = m_chamber(Ind);
